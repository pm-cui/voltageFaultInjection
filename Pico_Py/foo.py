...
def get_delay_duration():
    print("Enter delay duration(ns) btw 110 to 20,000: ", end = "")
    while True:
        delay_duration = readline()
        if len(delay_duration) != 0:
            try:
                # GLitch duration to be determined later. According to research, should be approximately 200ns-ish
                if (int(delay_duration) < 110 or int(delay_duration) > 20000):
                    raise Exception
                ...
                print("Error, please enter an integer of multiple 10 between 110 and 20000.")
...
def filter(data):
    data = data.replace("STM Starting...\n\r")
...
while True:
    ...
    if (j < 780):
        #~90ns of delay is due to current travel time(?)
        delay_cycles = (j - 140 )/20	# At least 4 cycles is being used for delay. Additionally, there is always a ~90ns delay before glitch occurs.
        ...
        delay_cycles = math.floor((j - (long_delay_cycles *320) - 110) / 20) #~90ns of delay. 10ns of delay to copy glitch duration into x
